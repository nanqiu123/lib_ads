...
/*
     ADS_ReadState_BuildFrame
	 输入参数：  (ADS_ReadState_Request_t *command, uint8_t *command_frame, uint16_t *command_lenth)
	 输出参数： 1成功， 0失败
*/
static char ADS_ReadState_BuildFrame(ADS_ReadState_Request_t *command, uint8_t *command_frame, uint16_t *command_lenth)
{
...
/*
     ReadState 解析帧
	 输入参数： uint8_t *command_frame, uint16_t command_lenth, ADS_ReadState_Request_t *command)
	 输出参数： 1成功， 0失败
*/
char ADS_ReadState_ResolevFrame(uint8_t *command_frame, uint16_t command_lenth, ADS_ReadState_Receive_t *command)
{
   uint16_t index = 0;
   if(command == NULL || command_frame == NULL) return 0;
   ...
   BigEndianHexToInterger_ByLittleEndian(&command_frame[index], (uint64_t *)&command->Receive.ADS_State, sizeof(command->Receive.ADS_State));
   index += sizeof(command->Receive.ADS_State);

   BigEndianHexToInterger_ByLittleEndian(&command_frame[index], (uint64_t *)&command->Receive.Device_State, sizeof(command->Receive.Device_State));
   index += sizeof(command->Receive.Device_State);
...
char ADS_ReadState(Ads_Handle_t  ctx ,AMS_ReadState_Receive_Dat_t *dat)
{
    ...
    uint8_t receive_buff[1024] = {0};
	 ...
	 command_send.Ams_Header.Command_Id = (uint16_t)ADS_CmdId_ReadState;
	 ...
	 if(0 == ADS_ReadState_BuildFrame(&command_send, send_buff, &send_lenth)) return 0;
	 ...
	 if(0 == ADS_ReadState_ResolevFrame(receive_buff, receive_lenth, &command_receive)) return 0;
