...
/*
     组帧
	 输入参数：  (ADS_Write_Request_t *command, uint8_t *command_frame, uint16_t *command_lenth)
	 输出参数： 1成功， 0失败
*/
static char ADS_Write_BuildFrame(ADS_Write_Request_t *command, uint8_t *command_frame, uint16_t *command_lenth)
{
   uint16_t index = 0;
...
/*
     Write 解析帧
	 输入参数： uint8_t *command_frame, uint16_t command_lenth, ADS_Write_Request_t *command)
	 输出参数： 1成功， 0失败
*/
char ADS_Write_ResolevFrame(uint8_t *command_frame, uint16_t command_lenth, ADS_Write_Receive_t *command)
{
   uint16_t index = 0;
   if(command == NULL || command_frame == NULL) return 0;
...
char ADS_Write(Ads_Handle_t  ctx ,ADS_GroupIndex_t gpoup_index, uint64_t index_offset, uint8_t *dat, uint64_t lenth)
{
    ...
    command_send.Ams_Tcp_Header.Command_Lenth = (uint32_t)(AMS_HEADER_BYTES + AMS_WRITE_REQUEST_BASE_BYTES + lenth);
	 ...
	 command_send.Ams_Header.Command_Id = (uint16_t)ADS_CmdId_Write;
    ...
    command_send.Request.Data = dat;
	 ...
	 if(0 == ADS_Write_BuildFrame(&command_send, send_buff, &send_lenth)) return 0;  
